...
if (isset($_GET['id'])) {
  ...
  $veriCek=$conn->prepare("SELECT * FROM rezerveler where rezerve_id= :rezerve_id and rezerve_rest_id = '{$restoran_id}'");
        ...
        <div class="card-header py-3">
          <div class="row">
            <div class="col-md-4 mb-2">
              <b> <span>Müşteri Adı Soyadı:</span> </b>
            ...
            <div class="col-md-4 mb-2 border">
              ...
              <h6 class="mt-2 font-weight-bold border p-2"><?=$bilgi['rezerve_siparis']; ?></h6>
            </div>
            <div class="col-md-4 mb-2">
              <b> <span class="">Sipariş Durumu:</span> </b>
              <h6 class="mt-2 font-weight-bold"><?php if($bilgi['rezerve_siparis_durum']==0){ echo '<span class="text-warning">Hazırlanıyor</span>'; } else { echo '<span class="text-success">Hazırlandı</span>'; } ?></h6>
            </div>
          </div>
        </div>
        <div class="card-body">
          <?php if($kullanici['yetkili_tip']==1){ ?>
            <?php if($bilgi['rezerve_durum']==0){  ?>
              <button value="<?php echo $bilgi['rezerve_id']; ?>" islem="rezerveTamamla"  class="btn btn-success btn-block btn-rezerve-tamamla"> <i class="fa fa-check"></i> Rezervasyon Onayla</button>
            <?php } else { ?>
              <button value="<?php echo $bilgi['rezerve_id']; ?>" islem="rezerveTamamla"  class="btn btn-danger btn-block btn-rezerve-tamamlama"> <i class="fa fa-times"></i> Rezervasyon Onaylama</button>
            <?php } ?>
          <?php } ?>


          <?php if($bilgi['rezerve_siparis_durum']==0){  ?>
            <button value="<?php echo $bilgi['rezerve_id']; ?>" islem="siparisTamamla"  class="btn btn-success btn-block btn-siparis-tamamla"> <i class="fa fa-check"></i> Sipariş Hazırlandı</button>
          <?php } else { ?>
            <button value="<?php echo $bilgi['rezerve_id']; ?>" islem="siparisTamamla"  class="btn btn-warning btn-block btn-siparis-tamamlama"> <i class="fa fa-times"></i> Sipariş Hazırlanıyor</button>
          <?php } ?>
...
<script type="text/javascript">
  ...
  $(".btn-siparis-tamamla").click(function(e) {
    swal({
      title: "Hazırlandı İşlemi",
      text: "İşlemi yapmak istediğinize emin misiniz?",
      icon: "success",
      buttons: true,
      dangerMode: true,
  ...
  $(".btn-siparis-tamamlama").click(function(e) {
    swal({
      title: "Hazırlanıyor İşlemi",
      text: "İşlemi yapmak istediğinize emin misiniz?",
      icon: "info",
      buttons: true,
      dangerMode: true,
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        $.ajax({
          type: "POST",
          url: "ajax/siparis_tamamlama.php",
          data: { 
            id: $(this).val(), 
          },
          success: function(result) {
